...
#need updating and do the update. I've also cobbled in a bit of code to update the local workstation that's running the script after the server updates.
...
# Here's the bit that's supposed to run on the executing desktop to update the clients.
Write-Host "Checking for updates on local machine..."

# Time to update your computer now.
#Here's the path to temporarily store the install file
$DownloadPath = "C:\windows\Temp"
#Use PDQStuff to get the current Deploy version
$DeployCurrentVersion = Get-PdqInstalledVersion -Product 'Deploy'
